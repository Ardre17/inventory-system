                ...
                <form method="POST" action="{{ route('raw-materials.update', $rawMaterial) }}">
                    ...
                    <div style="display:grid; grid-template-columns:1fr 1fr; gap:1rem; margin-bottom:1rem;">
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name', $rawMaterial->name) }}"
                    ...
                    <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:1rem; margin-bottom:1rem;">
                        <div>
                            ...
                            <input type="number" name="stock" value="{{ old('stock', $rawMaterial->stock) }}"
                        ...
                        <div>
                            ...
                            <input type="number" name="stock_min" value="{{ old('stock_min', $rawMaterial->stock_min) }}"
                                   step="0.001" min="0" style="width:100%; border:1px solid #d1d5db; border-radius:0.5rem; padding:0.5rem 0.75rem; font-size:0.9rem; box-sizing:border-box;" required>
                        </div>
                        <div>
                            <label style="display:block; font-weight:600; font-size:0.85rem; color:#374151; margin-bottom:0.4rem;">Costo *</label>
                            <input type="number" name="cost" value="{{ old('cost', $rawMaterial->cost) }}"
                    ...
                    <div style="display:grid; grid-template-columns:1fr 1fr; gap:1rem; margin-bottom:1rem;">
                        <div>
                            ...
                            <select name="supplier_id" style="width:100%; border:1px solid #d1d5db; border-radius:0.5rem; padding:0.5rem 0.75rem; font-size:0.9rem; box-sizing:border-box;">
                                ...
                                <option value="{{ $s->id }}" {{ old('supplier_id', $rawMaterial->supplier_id) == $s->id ? 'selected' : '' }}>{{ $s->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            <label style="display:block; font-weight:600; font-size:0.85rem; color:#374151; margin-bottom:0.4rem;">Lote</label>
                            <input type="text" name="lot" value="{{ old('lot', $rawMaterial->lot) }}"
                    ...
                    <div style="display:grid; grid-template-columns:1fr 1fr; gap:1rem; margin-bottom:1rem;">
                        <div>
                            ...
                            <input type="date" name="expiration_date"
                                   value="{{ old('expiration_date', $rawMaterial->expiration_date?->format('Y-m-d')) }}"
                                   style="width:100%; border:1px solid #d1d5db; border-radius:0.5rem; padding:0.5rem 0.75rem; font-size:0.9rem; box-sizing:border-box;">
                        </div>
                        <div>
                            <label style="display:block; font-weight:600; font-size:0.85rem; color:#374151; margin-bottom:0.4rem;">URL imagen</label>
                            <input type="text" name="image_url" value="{{ old('image_url', $rawMaterial->image_url) }}"
                    ...
                    <div style="margin-bottom:1.5rem;">
                        ...
                        <textarea name="description" rows="3"
                                  style="width:100%; border:1px solid #d1d5db; border-radius:0.5rem; padding:0.5rem 0.75rem; font-size:0.9rem; box-sizing:border-box;">{{ old('description', $rawMaterial->description) }}</textarea>
                    </div>
